...
- Todos os usuários visualizam o painel, mas somente o nível **Senior (Root)** — tipicamente o usuário `jerr` com `role = "super_admin"` — consegue executar ações. Caso um perfil **New** ou **Pro** toque em qualquer botão, o app exibe alerta "Permissão negada" e orienta a contatar o Analista Jerr por e-mail (variável `SANDBOX_CONTACT_EMAIL`, padrão `rama01rodriguez@duck.com`).
