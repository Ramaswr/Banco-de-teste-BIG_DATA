...
def read_sales_csv(path_or_buf, sep=';', encoding_candidates=('utf-8','latin1','cp1252')):
...
def clean_product_df(df):
    ...
    if "Categoria" not in df.columns:
        if df.shape[1] >= 5:
            ...
            raise ValueError("Formato do CSV de produto inesperado: colunas insuficientes.")
...
def clean_date_df(df):
    ...
    val_col  = next((c for c in df.columns if ('valor' in c.lower()) or ('value' in c.lower()) or ('preco' in c.lower())), None)
...
def aggregate_and_save(df_prod=None, df_date=None, output_folder='output', save_prefix=''):
    os.makedirs(output_folder, exist_ok=True)
    reports = {}
    if df_prod is not None:
        prod_agg = df_prod.groupby(['Produto','Codigo','Categoria'], dropna=False).agg(
            Quantidade_Total = ('Quantidade','sum'),
            Receita_Total = ('Valor','sum')
        ).reset_index().sort_values(by='Receita_Total', ascending=False)
        reports['produto_agg'] = prod_agg

        cat_agg = df_prod.groupby('Categoria', dropna=False).agg(
            Quantidade_Total = ('Quantidade','sum'),
            Receita_Total = ('Valor','sum')
        ).reset_index().sort_values(by='Receita_Total', ascending=False)
    ...
    if df_date is not None:
        ...
        daily = df_date.groupby('Data').agg(
            Quantidade_Total=('Quantidade','sum'),
            Receita_Total=('Valor','sum')
...
def run_etl(product_path=None, date_path=None, output_folder='output', sep=';', encoding_candidates=('utf-8','latin1','cp1252')):
    df_prod = None
    df_date = None
    if product_path:
        df_prod_raw = read_sales_csv(product_path, sep=sep, encoding_candidates=encoding_candidates)
        df_prod = clean_product_df(df_prod_raw)
    if date_path:
        df_date_raw = read_sales_csv(date_path, sep=sep, encoding_candidates=encoding_candidates)
        df_date = clean_date_df(df_date_raw)
    out_paths, reports = aggregate_and_save(df_prod=df_prod, df_date=df_date, output_folder=output_folder)
    return out_paths, reports


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='ETL para vendas (produto + data) - gera CSVs agregados')
    parser.add_argument('--product', help='Caminho para CSV de produto (categoria, codigo, produto, quantidade, valor)')
    parser.add_argument('--date', help='Caminho para CSV por data')
    parser.add_argument('--output', default='output', help='Pasta de saída (será criada)')
    parser.add_argument('--sep', default=';', help='Separador dos CSVs de entrada (padrão: ; )')
    args = parser.parse_args()

    out, reports = run_etl(product_path=args.product, date_path=args.date, output_folder=args.output, sep=args.sep)
